...
function sejaefipixdf_config() {
    ...
    return array(
        ...
        'ModoOperacao' => array(
            'FriendlyName' => 'Em Produção',
            'Type' => 'dropdown',
            'Options' => array(
                '1' => 'Sim',
                '0' => 'Não',
            ),
            'Description' => 'Colocar modulo em produção (Gerar Faturas Reais)',
        ),
        ...
        'ClientIdProducao' => array(
            'FriendlyName' => 'Client_Id de Produção',
            'Type' => 'text',
            'Default' => '',
            'Description' => 'Coloque seu Client_Id de Produção',
        ),
        
        'ClientSecretProducao' => array(
            'FriendlyName' => 'Client_Secret de Produção',
            'Type' => 'text',
            'Default' => '',
            'Description' => 'Coloque seu Client_Secret de Produção',
        ),
        
        'CertificadoProducao' => array(
            'FriendlyName' => 'Caminho do Certificado de Produção',
            'Type' => 'text',
            'Default' => '',
            'Description' => 'Coloque o caminho completo do Certificado de Produção',
        ),
...
function sejaefipixdf_config_validate($params)
{
    ...
    if ($params['ChavePix'] == '') {
        throw new \Exception('O campo Chave Pix não foi preenchido ' );
    }
    
    if ($params['ClientIdProducao'] == '') {
        throw new \Exception('O campo Cliente_ID de Produção não foi preenchido');
    }
    ...
    if ($params['CertificadoProducao'] == '' || !file_exists($params['CertificadoProducao'])) {
        throw new \Exception('Certificado de Produção não Localizado no Servidor');
    }

    if($params['ModoOperacao']=="0"){
        
        if ($params['ClientIdSandbox'] == '') {
            throw new \Exception('O campo Cliente_ID de Sandbox não foi preenchido');
        }
        ...
        if ($params['CertificadoSandbox'] == '' || !file_exists($params['CertificadoSandbox'])) {
            throw new \Exception('Certificado de Sandbox não Localizado no Servidor');
        }
    }
...
function sejaefipixdf_link($params) {
    ...
    //verifica se já existe a fatura no BD
    try {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => $url_api_gerencianet . "/oauth/token", // Rota base, homologação ou produção
     ...
     * */
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url_api_gerencianet . "/v2/gn/config", // Rota base, homologação ou produção
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
      ...
      * */
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url_api_gerencianet . "/v2/webhook/" . $chave_pix, // Rota base, homologação ou produção
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        */
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url_api_gerencianet . "/v2/cob/" . $FaturaTexto, // Rota base, homologação ou produção
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        if($sandbox){
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => $url_api_gerencianet . "/v2/loc/" . $IdLocationPix . "/qrcode", // Rota base, homologação ou produção
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
